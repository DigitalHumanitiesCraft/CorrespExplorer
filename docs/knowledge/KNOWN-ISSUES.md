...
Geänderte Dateien: style.css, CONTEXT-MAP.md

Problem:
- tokens.css: 380px (var(--sidebar-width))
- style.css: hardcodierte -280px fuer mobile hide
- design.md: veraltete Werte dokumentiert

Lösung:
- style.css verwendet nun calc(-1 * var(--sidebar-width)) statt hardcodierter Werte
- wissenskorb.css verwendete bereits var(--sidebar-width)
- CONTEXT-MAP.md Dokumentation aktualisiert
...
2. **HIGH**: Sidebar-Breite konsolidieren - BEHOBEN
3. **MEDIUM**: VIEWS-Konstante ergänzen - BEHOBEN
4. **MEDIUM**: README.md aktualisieren - BEHOBEN
5. **LOW**: design.md mit tokens.css synchronisieren (Referenz-Genauigkeit) - OFFEN
6. **LOW**: Breakpoints und Spacing vereinheitlichen (Edge-Cases) - OFFEN
